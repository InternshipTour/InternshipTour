...
class IndexController extends BaseController
{
    ...
    private function group_opt($aKeyword=""){
        ...
        if ($aOrder == 'activity') {
            $this->assign('order', L('sort_by_activity'));
            ...
            $this->assign('order', L('sort_by_member'));
        } else {
            $aOrder = 'create_time';
            $this->assign('order', L('sort_by_time'));
        }
        if (!empty($aCate)) {
        ...
        if ($aUid != 0) {
            ...
            $this->assign('name', get_nickname($aUid) . L('of_group'));
    ...
    public function group()
    {
        ...
        if ($aId != 0) {
            ...
            $this->setTitle(L('module'));
            $this->assign('group', array('title' => L('group_group')));
        }
    ...
    public function create()
    {
        if (IS_POST) {
            ...
            $aMemberAlias = I('post.member_alias', L('member'), 'text');



            if (empty($aTitle)) {
                $this->error(L('error_group_name'));
            }
            if (utf8_strlen($aTitle) > 20) {
                $this->error(L('error_title_length'));
            }
            if ($aGroupType == -1) {
                $this->error(L('error_category'));
            }
            if (empty($aDetail)) {
                $this->error(L('error_intro'));
            }
            $model = D('Group');
            ...
            if ($isEdit) {
                ...
                $this->checkAuth('Group/Index/editGroup', get_group_admin($aGroupId), L('authority_edit_not'));
            } else {

                $this->requireLimit();

                $this->checkActionLimit('add_group', 'Group', 0, is_login(), true);
                $this->checkAuth('Group/Index/addGroup', -1, L('authority_add_not'));
            }
            ...
            if ($isEdit) {
                ...
                if (!$result) {
                    $this->error(L('error_create_group') . $model->getError());
                }
                $group_id = $result;
            ...
            if ($need_verify) {
                $message = L('tip_create_group_success');
                // 发送消息
                D('Message')->sendMessage(1,L('create_group_audit'), get_nickname(is_login()) . L('created_group_1')."【{$aTitle}】".L('created_group_2'),  'admin/group/unverify');
            ...
            if (D('Module')->checkInstalled('Weibo')) {
                ...
                if ($isEdit && check_is_in_config('edit_group', modC('GROUP_SEND_WEIBO', 'add_group,edit_group', 'GROUP'))) {
                    D('Weibo/Weibo')->addWeibo(is_login(), L('i_change_group')."【" . $aTitle . "】：" . $postUrl);
                }
                if (!$isEdit && check_is_in_config('add_group', modC('GROUP_SEND_WEIBO', 'add_group,edit_group', 'GROUP'))) {
                    D('Weibo/Weibo')->addWeibo(is_login(), L('i_create_group')."【" . $aTitle . "】：" . $postUrl);
                }

            }

            //显示成功消息
            $message = $isEdit ? L('edit_success') : L('publish_success');
    ...
    private function requireLimit(){
        ...
        if($count >= $limit){
            $this->error(L('group__number_top').$limit);
        }
    }
    public function my()
    {
    ...
    public function detail()
    {
        ...
        $this->setTitle('{$post.title|op_t} '.L('post_bar'));
    ...
    public function edit()
    {
        ...
        if ($isEdit) {
            ...
            $this->checkAuth('Group/Index/edit', get_post_admin($aPostId), L('you_edit_limit'));
            $post = D('GroupPost')->getPost($aPostId);
        } else {
            if (is_joined($aGroupId) != 1) {
                $this->error(L('you_post_limit'));
            }
            $this->checkAuth('Group/Index/addPost', -1, L('you_add_limit'));
        ...
        $this->setTitle('{$title}');
        $this->assign('title', $isEdit ? L('edit_post') : L('publish_post'));
        $this->assign('post', $post);
    ...
    public function doEdit()
    {
        ...
        if (empty($aGroupId)) {
            $this->error(L('please_title'));
        }
        if (empty($aTitle)) {
            $this->error(L('please_title'));
        }
        if (empty($aContent)) {
            $this->error(L('please_content'));
        }
        ...
        if ($isEdit) {
            ...
            if (!$result) {
                $this->error(L('edit_fail') . $model->getError());
            }
        } else {
        ...
        //显示成功消息
        $message = $isEdit ? L('edit_success') : L('publish_success') . cookie('score_tip');
        ...
        if (D('Module')->checkInstalled('Weibo')) {
            ...
            if (D('Common/Module')->isInstalled('Weibo')) { //安装了微博模块
                if ($isEdit && check_is_in_config('edit_group_post', modC('GROUP_POST_SEND_WEIBO', 'add_group_post,edit_group_post', 'GROUP'))) {
                    D('Weibo/Weibo')->addWeibo(is_login(), L('at_group')."【{$group['title']}】".L('update_a_post')."【" . $post['title'] . "】：" . $postUrl, $type, $feed_data);
                }
                if (!$isEdit && check_is_in_config('add_group_post', modC('GROUP_POST_SEND_WEIBO', 'add_group_post,edit_group_post', 'GROUP'))) {
                    D('Weibo/Weibo')->addWeibo(is_login(), L('at_group')."【{$group['title']}】".L('publish_a_post')."【" . $post['title'] . "】：" . $postUrl, $type, $feed_data);
                }
            }
        }
    }
    ...
    public function doBookmark()
    {
        ...
        $this->requirePostExists($aPostId);
        $this->checkAuth(null, -1, L('authority_collect'));
    ...
    public function recommend()
    {
        ...
        $this->requirePostExists($aPostId);
        $this->checkAuth(null, get_group_admin($group_id), L('authority_set_top'));
    ...
    public function delPostReply()
    {
        ...
        $this->requireLogin();
        $this->checkAuth(null, get_reply_admin($ReplyId), L('authority_delete'));
    ...
    public function doSendLzlReply()
    {
        ...
        if (!$result) {
            $this->error(L('release_fail') . $model->getError());
        }
        ...
        $res['status'] = 1;
        $res['info'] = L('reply_success') . cookie('score_tip');
        $this->ajaxReturn($res);
    }
    ...
    public function editReply()
    {
        ...
        if (IS_POST) {
            ...
            if ($res) {
                ...
                $this->success(L('edit_reply_success'), U('Group/Index/detail', array('id' => $post['post_id'])));
            } else {
                $this->error(L("edit_reply_fail"));
            }
        } else {
            if ($aReplyId) {
            ...
            $this->setTitle(L('title_edit_reply'));
            //显示页面
    ...
    public function doReply()
    {
        ...
        if (!$result) {
            $this->error(L('reply_fail') . $model->getError());
        }
        //显示成功消息
        $this->success(L('reply_success') . cookie('score_tip'), 'refresh');
    ...
    public function attend()
    {
        ...
        //判断是否已经加入
        if (is_joined($aGroupId) == 1) {
            $this->error(L('has_in_this_group'));
        }
        // 已经加入但还未审核
        ...
        if ($group['type'] == 1) {
            ...
            $info = L('wait_admin_audit');
            // 发送消息
            D('Message')->sendMessage($group['uid'], L('join_group_audit'),get_nickname($uid) . L('ask_for_group')."【{$group['title']}】", 'group/Manage/member', array('group_id' => $aGroupId, 'status' => 0), $uid);
        } else {
            // 群组为公共的
            $data['status'] = 1;
            $res = D('GroupMember')->addMember($data);
        ...
        if ($res) {
            ...
            $this->success(L('success_join') . $info, 'refresh');
        } else {
            $this->error(L('fail_join'));
        }
    ...
    public function quit()
    {
        ...
        $group = D('Group')->getGroup($aGroupId);
        if ($group['uid'] == $uid) {
            $this->error(L('creator_cannot_quit'));
        }
        // 判断是否在该群组内
        if (is_joined($aGroupId) == 0) {
            $this->error(L('not_in_group'));
        }
        ...
        if ($res) {
            ...
            S('group_is_join_' . $aGroupId . '_' . $uid, null);
            S('group_member_count_' . $group['id'], null);
            $this->success(L('success_quit'), 'refresh');
        } else {
            $this->error(L('fail_quit'));
        }
    }
    ...
    public function groupInvite()
    {
        ...
        if (IS_POST) {
            ...
            $group = D('Group')->getGroup($aGroupId);
            foreach ($uids as $uid) {
                D('Message')->sendMessage($uid, '', get_nickname(is_login()) . L('invite_to_group')."【{$group['title']}】  <a class='ajax-post' href='" . U('group/index/attend', array('group_id' => $aGroupId)) . "'>".L('invite_to_group')."</a>",  'group/index/group', array('id' => $aGroupId), is_login());
            }

            $result = array('status' => 1, 'info' => L('success_invite'));
    ...
    public function deletePost()
    {
        ...
        if(!$post){
            $this->ajaxReturn(array('status'=>0,'info'=>L('post_not_exist'),'url'=>U('Group/Index/groups')));
        }
        $this->checkAuth('Group/Index/deletePost',get_admin_ids($aPostId,3,0),L('authority_delete_limit'));
        $res=$postModel->where($map)->setField('status',-1);
        if($res){
            $this->ajaxReturn(array('status'=>1,'info'=>L('delete_success'),'url'=>U('Group/Index/group',array('id'=>$post['group_id']))));
        }else{
            $this->ajaxReturn(array('status'=>0,'info'=>L('delete_fail').$postModel->getError()));
        }
    }
