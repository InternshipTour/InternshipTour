...
class ConfigController extends AdminController {
	...
	public function index() {
		...
		// $map=
		$list = $this->lists ( 'Config', $map, 'sort,id desc' );
		// 记录当前列表页的cookie
	...
	public function website() {
		...
		$builder->group ( '页脚信息', array (
				'ABOUT_US',
				'SUBSCRIB_US',
				'COPY_RIGHT' 
		) );
		
		$builder->group ( '跳转页面', array (
				'JUMP_BACKGROUND',
				'SUCCESS_WAIT_TIME',
				'ERROR_WAIT_TIME' 
		) );
		...
		$builder->group ( '性能设置', array (
				'GET_INFORMATION',
				'GET_INFORMATION_INTERNAL' 
		) );
		$builder->group ( '上传配置', array (
				'PICTURE_UPLOAD_DRIVER',
				'DOWNLOAD_UPLOAD_DRIVER' 
		) );
