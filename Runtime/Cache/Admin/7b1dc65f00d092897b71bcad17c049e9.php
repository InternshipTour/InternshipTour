    ...
    <li class="<?php echo ($fieldRegisterClass); ?>"><a href="<?php echo U('Role/configField',array('id'=>$this_role['id'],'type'=>1));?>">注册时填写资料配置</a></li>
            ...
            <div class="col-xs-2 text-right">
                <select name="role" class="form-control">
                    <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Role/configScore',array('id'=>$vo['id']));?>" <?php if(($vo['id']) == $this_role['id']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="node-list">
            ...
            <div class="tab-pane in">
                <form action="/internship-tour/index.php?s=/admin/role/configscore/id/1.html" enctype="application/x-www-form-urlencoded" method="POST" class="form-horizontal auth-form">
                    <?php if(is_array($score_keys)): $i = 0; $__LIST__ = $score_keys;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$score): $mod = ($i % 2 );++$i;?><label class="item-label"><?php echo (htmlspecialchars($score["title"])); ?><span>（单位：<?php echo (htmlspecialchars($score["unit"])); ?>）</span></label>
                        <div class="controls ">
                            <input type="text" name="score<?php echo ($score["id"]); ?>" value="<?php echo ($score["value"]); ?>" class="text input-large form-control"style="width: 400px"/>
...
<script type="text/javascript">
    ...
    <script type="text/javascript" charset="utf-8">
        ...
        //导航高亮
        highlight_subnav('<?php echo U('Role/configscore');?>');
        }(jQuery);
    </script>
