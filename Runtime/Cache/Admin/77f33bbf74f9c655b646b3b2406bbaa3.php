...
<div class="panel-main" style="float:left;">
    ...
    <div class="with-padding" style="width:700px;">
        <div class="alert alert-info">
            <p>当前的系统版本号：<strong><?php echo ($currentVersion["name"]); ?></strong></p>
        </div>
        <p><strong class="text-success">1.下载更新包，获取更新文件列表 -》 2.本地文件对比 -》 3.代码升级 -》 4.数据库升级 -》升级完成</strong></p>
        <h4>
            恭喜，升级成功，目前版本为 <?php echo ($currentVersion["name"]); ?>
        </h4>
        ...
        <div class="row">
            <div class="col-xs-7">
                <a id="compare_btn" class="btn btn-success btn-block"
                  href="<?php echo U('update');?>">返回自动升级首页</a>
            </div>
    ...
    <script>
        function updateDB(obj) {
            if (!confirm('确定导入？强烈建议在导入之前进行数据库备份。')) {
                ...
                $.post('/internship-tour/index.php?s=/admin/cloud/finish.html', function (msg) {
                    if (msg.status) {
                        $(obj).removeAttr('disabled');
                        toast.hideLoading();
                        toast.success('数据库升级成功。');
                        $(obj).text('升级数据库成功，进入下一步');
                        $(obj).attr('href',"<?php echo U('finish');?>");
