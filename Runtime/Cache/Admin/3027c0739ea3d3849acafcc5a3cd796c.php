...
<div class="panel-main" style="float:left;">
	...
	<div class="main-title clearfix ">
		<h2><?php if($_GET['_action'] == 'add'): ?>新增<?php else: ?>编辑<?php endif; ?>友情连接</h2>
	</div>
	<!-- 表单 -->
	<form id="form" action="<?php echo addons_url('SuperLinks://SuperLinks/update');?>" method="post" class="form-horizontal">
		<!-- 基础文档模型 -->
		<div id="tab1" class="tab-pane with-padding in tab1">
			<div class="form-item cf">
				...
				<div class="controls">
					<input type="text" class="text form-control form-input-width" name="title" value="<?php echo ($info["title"]); ?>">
				</div>
			</div>
			<div class="form-item cf">
				<label class="item-label">连接类型</label>
				<div class="controls ">
				  <?php if($_GET['_action'] == 'add'): ?><label class="radio-inline"><input type="radio" class="radio-inline"  name="type" value="2" checked="checked"  onclick="javascript:hide_img()">普通连接</label>
					<label class="radio-inline"><input type="radio" class="radio-inline" name="type" value="1"onclick="javascript:show_img()">图片连接</label>

				  <?php else: ?>
					<label class="checkbox-inline"><input type="radio" name="type" value="1" <?php if($info["type"] == 1): ?>checked="checked"<?php endif; ?> onclick="javascript:show_img()">图片</label>
					<label class="checkbox-inline"><input type="radio" name="type" value="2" <?php if($info["type"] == 2): ?>checked="checked"<?php endif; ?> onclick="javascript:hide_img()">普通</label>
				  	<script>
				  		$(function(){
				  			<?php if($info["type"] == 1): ?>show_img();
						  	<?php else: ?>
						  		hide_img();<?php endif; ?>
				  		});				  	
				  	</script><?php endif; ?>
			...
			<div class="form-item cf">
				...
				<div class="controls">
					<input type="text" class="text  form-control form-input-width" name="link" value="<?php echo ($info["link"]); ?>">
				</div>
			</div>
			<div class="form-item cf " style="display: none" id="img_area">
				<label class="item-label">站点图片</label>
				<div class="controls">
                    <?php echo W('Common/UploadImage/render',array(array('name'=>'cover_id','id'=>'cover_id','value'=>$info['cover_id'])));?>
