...
<?php switch($field["type"]): case "text": ?><input type="text" name="<?php echo ($field["name"]); ?>" value="<?php echo (htmlspecialchars($field["value"])); ?>"
    ...
    <?php case "icon": ?><div class='icon-chose' title="选择icon" style="width: 400px;">
            <select name="<?php echo ($field["name"]); ?>" title="选择icon" class="chosen-icons" data-value="<?php echo ((isset($field["value"]) && ($field["value"] !== ""))?($field["value"]):'icon-star'); ?>"></select>
        </div>
    ...
    <?php case "singleImage": ?><div class="controls">
            ...
            <div class="upload-img-box">
                ...
                <?php if(!empty($field["value"])): ?><div class="each">
                    <a href="<?php echo (get_cover($field["value"],'path')); ?>" title="点击查看大图">
                        <img src="<?php echo (get_cover($field["value"],'path')); ?>">
                    </a>
                        <div class="text-center opacity del_btn" ></div>
                        <div onclick="admin_image.removeImage($(this),'<?php echo ($field["value"]); ?>')"  class="text-center del_btn">删除</div>
                    </div><?php endif; ?>
                </div>
            </div>
        </div>
    ...
    <?php case "time": $importDatetimePicker = true; if(!$field['value']){ $field['value'] = time(); } ?>
        ...
        <input type="text" data-field-name="<?php echo ($field["name"]); ?>" class="text input-large form-time time form-control"
               style="width: 400px" value="<?php echo (time_format($field["value"],'H:i')); ?>" placeholder="请选择时间"/><?php break;?>
    <?php case "date": $importDatetimePicker = true; if(!$field['value']){ $field['value'] = time(); } ?>
        ...
        <input type="text" data-field-name="<?php echo ($field["name"]); ?>" class="text input-large form-date time form-control"
               style="width: 400px" value="<?php echo (time_format($field["value"],'Y-m-d')); ?>" placeholder="请选择时间"/><?php break;?>
    <?php case "datetime": $importDatetimePicker = true; if(!$field['value']){ $field['value'] = time(); } ?>
        ...
        <input type="text" data-field-name="<?php echo ($field["name"]); ?>" class="text input-large form-datetime time form-control"
               style="width: 400px" value="<?php echo (time_format($field["value"])); ?>" placeholder="请选择时间"/><?php break;?>
    ...
    <?php case "dataselect": ?><input type="text" name="<?php echo ($field["name"]); ?>" id="<?php echo ($field["name"]); ?>" value="<?php echo (htmlspecialchars($field["value"])); ?>"
               class="text input-large form-control" style="width: 400px;display:inline-block;"/><input class="btn" style="margin-left:10px" type="button" value="选择" onclick="openwin('<?php echo ($field["opt"]); ?>','600','500')">
			     <script type="text/javascript">
						//弹出窗口
...
<script type="text/javascript">
    ...
    <?php if($importDatetimePicker): ?><link href="/internship-tour/Application/Admin/Static/zui/lib/datetimepicker/datetimepicker.css" rel="stylesheet" type="text/css">
        <script type="text/javascript" src="/internship-tour/Application/Admin/Static/zui/lib/datetimepicker/datetimepicker.min.js"></script>
    ...
    <?php if($colorPicker): ?><script type="text/javascript" src="/internship-tour/Application/Admin/Static/js/jquery.simple-color.js"></script>
        <script>
            $(function(){
