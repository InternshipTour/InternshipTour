    ...
    <li class="<?php echo ($fieldRegisterClass); ?>"><a href="<?php echo U('Role/configField',array('id'=>$this_role['id'],'type'=>1));?>">注册时填写资料配置</a></li>
        ...
        <div class="col-xs-2 text-right">
            <select name="role" class="form-control">
                <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Role/configavatar',array('id'=>$vo['id']));?>" <?php if(($vo['id']) == $this_role['id']): ?>selected<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </div>
    </div>
    <div class="with-padding" style="margin-top: 20px;">
        <form action="/internship-tour/index.php?s=/admin/role/configavatar/id/1.html" enctype="application/x-www-form-urlencoded" method="POST" class="form-horizontal avatar-form ajax-form">
            <input type="hidden" name="id" value="<?php echo ($this_role["id"]); ?>"/>
            <input class="attach" type="hidden" name="avatar_id" value="<?php echo ($this_role["avatar"]); ?>"/>
            <input class="attach" type="hidden" name="set_null" value="0"/>
            <label class="item-label" style="font-size: 15px;font-weight: 400;">上传默认头像<span style="font-size: 12px;margin-left: 20px;color: rgb(253, 67, 67);">请上传256px*256px的png头像</span></label>
            <div class="controls">
                <div id="upload_single_image" style="padding-bottom: 5px;">选择图片</div>
                <?php if(!isset($this_role['avatar'])): ?><div data-role="avatar-block" style="display:none;">

                        <div class="avatar">
                            <a href="<?php echo (getthumbimagebyid($this_role["avatar"],'256','256')); ?>" data-role="avatar256" title="点击查看大图">
                                <img src="<?php echo (getthumbimagebyid($this_role["avatar"],'256','256')); ?>">
                            </a>
                            <div class="img_tips">256px*256px</div>
                        </div>
                        <div class="avatar">
                            <a href="<?php echo (getthumbimagebyid($this_role["avatar"],'128','128')); ?>" data-role="avatar128" title="点击查看大图">
                                <img src="<?php echo (getthumbimagebyid($this_role["avatar"],'128','128')); ?>">
                            </a>
                            <div class="img_tips">128px*128px</div>
                        </div>
                        <div class="avatar">
                            <a href="<?php echo (getthumbimagebyid($this_role["avatar"],'64','64')); ?>" data-role="avatar64" title="点击查看大图">
                                <img src="<?php echo (getthumbimagebyid($this_role["avatar"],'64','64')); ?>">
                            </a>
                            <div class="img_tips">64px*64px</div>
                        </div>
                        <div class="avatar">
                            <a href="<?php echo (getthumbimagebyid($this_role["avatar"],'32','32')); ?>" data-role="avatar32" title="点击查看大图">
                                <img src="<?php echo (getthumbimagebyid($this_role["avatar"],'32','32')); ?>">
                            </a>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var uploader= WebUploader.create({
                ...
                swf: 'Uploader.swf',
                // 文件接收服务端。
                server: "<?php echo U('Role/uploadPicture',array('session_id'=>session_id()));?>",
                // 选择文件的按钮。可选。
        ...
        //导航高亮
        highlight_subnav('<?php echo U('Role/configavatar');?>');
    </script>
