...
<div class="panel-main" style="float:left;">
    ...
    <div class="with-padding">
        <a href="<?php echo U('hooks');?>" class="btn">钩子管理</a>

        <a href="<?php echo U('create');?>" class="btn btn-primary">快速创建插件</a>
    </div>


    <div class="with-padding">
        <ul class="nav nav-secondary">
            <li id="type_all"><a href="<?php echo U('index',array('type'=>'all'));?>">全部</a></li>
            <li id="type_yes"><a href="<?php echo U('index',array('type'=>'yes'));?>">未安装</a></li>
            <li id="type_no"><a href="<?php echo U('index',array('type'=>'no'));?>">已安装</a></li>
            <script>
        ...
        <div class="cards plugin-list" style="padding: 15px">
            <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="pull-left" style="width: 240px;margin-right: 15px">
                        <div class="card">
                            <div class="media-wrapper"><a href="<?php echo U('config',array('id'=>$vo['id']));?>"><img src="/internship-tour/Application/Admin/Static/images/plugin.png" alt=""></a></div>
                            <span class="caption"><?php echo ($vo["name"]); ?></span>
                            <div class="card-heading">
                                <span class="pull-right"><span class="label" ><?php echo ((isset($vo["status_text"]) && ($vo["status_text"] !== ""))?($vo["status_text"]):"未安装"); ?></span> <span class="label label-success"><?php echo ($vo["version"]); ?></span></span>
                            ...
                            <div class="card-actions">
                                ...
                                <div class="btn-group">
                                    ...
                                    <?php if(empty($vo["uninstall"])): $class = get_addon_class($vo['name']); if(!class_exists($class)){ $has_config = 0; }else{ $addon = new $class(); $has_config = count($addon->getConfig()); } ?>
                                        <?php if ($has_config): ?>
                                        <a class="btn btn-primary" href="<?php echo U('config',array('id'=>$vo['id']));?>">设置</a>
                                        <?php endif ?>
                                        <?php if ($vo['status'] >=0): ?>
                                        <?php if(($vo["status"]) == "0"): ?><a class="btn btn-primary ajax-get"
                                               href="<?php echo U('enable',array('id'=>$vo['id']));?>">启用</a>
                                            <?php else: ?>
                                            <a class="btn btn-info ajax-get"
                                               href="<?php echo U('disable',array('id'=>$vo['id']));?>">禁用</a><?php endif; ?>
                                        <?php endif ?>
                                        
                                            <a class="btn btn-danger ajax-get"
                                               href="<?php echo U('uninstall?id='.$vo['id']);?>">卸载</a>
                                            
                                        <?php else: ?>
                                        <a class="btn btn-success ajax-get"
                                           href="<?php echo U('install?addon_name='.$vo['name']);?>">安装</a><?php endif; ?>
                                </div>
                            </div>
                        </div>
                    </div><?php endforeach; endif; else: echo "" ;endif; ?>
                <?php else: ?>
                <div class="text-center"> aOh! 暂时还没有内容!</div><?php endif; ?>
