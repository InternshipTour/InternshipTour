    ...
    <li class="<?php echo ($fieldRegisterClass); ?>"><a href="<?php echo U('Role/configField',array('id'=>$this_role['id'],'type'=>1));?>">注册时填写资料配置</a></li>
            ...
            <div class="col-xs-2 text-right">
                <select name="role" class="form-control">
                    <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Role/configfield',array('id'=>$vo['id']));?>"
        ...
        <div>
            ...
            <div class="node-list in">
                <form action="/internship-tour/index.php?s=/admin/role/configfield/id/1.html" enctype="application/x-www-form-urlencoded" method="POST" class="form-horizontal auth-form">
                    <?php if(!empty($field_list)): if(is_array($field_list)): $i = 0; $__LIST__ = $field_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><dl class="checkmod">
                                    ...
                                    <?php if(isset($group['field_list'])): if(is_array($group['field_list'])): $i = 0; $__LIST__ = $group['field_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><div class="filed_check">
                                                <div>
                                                    <label class="checkbox"
                                                           title="id：<?php echo ($field['id']); ?>  字段类型：<?php echo ($field['form_type']); ?>">
                                                        <input class="auth_fields fields_row" type="checkbox"
                                                               name="fields[]" value="<?php echo ($field["id"]); ?>"/><?php echo ($field["field_name"]); ?>
                                                    </label>
                                                </div>
                        ...
                        <?php else: ?>
                        <div style="margin: 10px;color: rgb(255, 81, 81);"> 请先到 用户》用户管理》扩展资料 中<a href="<?php echo U('admin/user/profile');?>" title="" target="_blank" style="color: rgb(255, 81, 81);text-decoration: underline;">添加用户扩展资料</a>~</div><?php endif; ?>
