...
<div class="panel-main" style="float:left;">
    ...
    <div class="with-padding" style="width:800px;">
        <div class="alert alert-info">
            <p>当前远程最新版本：<strong><?php echo ($version["0"]["name"]); ?></strong></p>

            <p>当前的系统版本号：<strong><?php echo ($currentVersion); ?></strong></p>
        </div>
        <p class="alert alert-danger">在安装更新之前，请先<a target="_blank" href="<?php echo U('database/index?type=export');?>">备份数据库</a>，备份网站源码，以防数据或者二次开发内容丢失。
        </p>
        <p class="alert alert-danger">请先到后台<a target="_blank" href="<?php echo U('config/group');?>">系统设置</a>页面，关闭网站。如果不希望自动更新，请手动<strong>关闭自动更新</strong>。</p>
        ...
        <table class="table table-hover">
            ...
            <?php if(is_array($version)): $i = 0; $__LIST__ = $version;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="<?php echo ($vo["class"]); ?>">
                    ...
                    <td>
                        <a data-remote="<?php echo U('Cloud/version',array('name'=>$vo['name']));?>" data-toggle="modal" data-title="<?php echo ($vo["title"]); ?>-<?php echo ($vo["name"]); ?> 更新日志">查看详情</a>
                    </td>
                    <td>
                        <?php if($vo[name] == $nextVersion['name']): ?><a href="<?php echo U('getFileList',array('version'=>$vo['name']));?>">自动升级</a>&nbsp;|&nbsp;<a href="<?php echo ($cloud); echo cloudU('Appstore/Update/download',array('number'=>$vo['number']));?>" target="_blank">手动下载</a>
                            <?php else: ?>
                            <?php if(version_compare($vo['name'],$currentVersion) == 0): ?><i class="icon-arrow-left"></i> 当前版本
                                <?php else: ?>
                                <?php if(version_compare($vo['name'],$currentVersion) == -1): ?>历史版本
                                    <?php else: ?>
                                    未来版本<?php endif; endif; endif; ?>
                      </td>
