    ...
    <li class="<?php echo ($fieldRegisterClass); ?>"><a href="<?php echo U('Role/configField',array('id'=>$this_role['id'],'type'=>1));?>">注册时填写资料配置</a></li>
            ...
            <div class="col-xs-2 text-right">
                <select name="role" class="form-control">
                    <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Role/configRank',array('id'=>$vo['id']));?>" <?php if(($vo['id']) == $this_role['id']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="node-list">
            ...
            <div class="tab-pane in">
                <form action="/internship-tour/index.php?s=/admin/role/configrank/id/1.html" enctype="application/x-www-form-urlencoded" method="POST" class="form-horizontal auth-form">
                    <?php if(!empty($can_apply)): ?><dl class="checkmod">
                            <dt class="hd">
                                <label class="checkbox"><input class="auth_rank rank_all" type="checkbox">用户可申请头衔</label>
                            </dt>
                            <dd class="bd">
                                <?php if(is_array($can_apply)): $i = 0; $__LIST__ = $can_apply;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rank): $mod = ($i % 2 );++$i;?><div class="rank_check">
                                        <div>
                                            <label class="checkbox" title="id：<?php echo ($rank['id']); ?>  创建人：<?php echo ($rank['name']); ?>  创建时间：<?php echo (time_format($rank['create_time'],'Y-m-d')); ?>">
                                            <input class="auth_rank rank_row" type="checkbox" name="ranks[]" value="<?php echo ($rank['id']); ?>"/><?php echo ($rank['title']); ?>
                                                <?php if(empty($rank["label_content"])): ?><img src="<?php echo (getthumbimagebyid($rank['logo'])); ?>"/><?php else: ?>
                                                    <span class="label label-badge rank-label" title="<?php echo ($rank["title"]); ?>" style="line-height: 6px;height: 18px;vertical-align: middle;text-align: center; background: <?php echo ($rank["label_bg"]); ?> !important;color:<?php echo ($rank["label_color"]); ?> !important;"><?php echo ($rank["label_content"]); ?></span><?php endif; ?>
                                            </label>
                                        </div>
                    ...
                    <?php if(!empty($un_apply)): ?><dl class="checkmod">
                            <dt class="hd">
                                <label class="checkbox"><input class="auth_rank rank_all" type="checkbox">用户不可申请头衔</label>
                            </dt>
                            <dd class="bd">
                                <?php if(is_array($un_apply)): $i = 0; $__LIST__ = $un_apply;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rank): $mod = ($i % 2 );++$i;?><div class="rank_check">
                                        <div>
                                            <label class="checkbox" title="id：<?php echo ($rank['id']); ?>  创建人：<?php echo ($rank['name']); ?>  创建时间：<?php echo (time_format($rank['create_time'],'Y-m-d')); ?>">
                                                <input class="auth_rank rank_row" type="checkbox" name="ranks[]" value="<?php echo ($rank['id']); ?>"/><?php echo ($rank['title']); ?><img src="<?php echo (getthumbimagebyid($rank['logo'])); ?>"/>
                                            </label>
                                        </div>
                                    </div><?php endforeach; endif; else: echo "" ;endif; ?>
                                <div class="clearfix"></div>
                    ...
                    <?php if(count($un_apply)||count($can_apply)): ?><label class="item-label">头衔颁发原因</label>
                        <div class="controls">
                            <textarea name="reason" style="width: 500px;min-height: 100px;"><?php echo ($reason); ?></textarea>
                        </div>
                        <?php else: ?>
                        <div style="color: #B5B4B4;font-size: 16px;margin: 10px;">
                            请先到 用户》头衔管理》头衔列表 中<a href="<?php echo U('admin/rank/editrank');?>" target="_blank" title="立刻去添加头衔" style="color: #B5B4B4;text-decoration: underline;">添加头衔</a>~
                        </div><?php endif; ?>
...
<script type="text/javascript">
    ...
    <script type="text/javascript" charset="utf-8">
        +function($){
            var ranks = [<?php echo ($this_role["ranks"]); ?>];
        ...
        //导航高亮
        highlight_subnav('<?php echo U('Role/configrank');?>');
        }(jQuery);
    </script>
