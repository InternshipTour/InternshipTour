    ...
    <li class="<?php echo ($fieldRegisterClass); ?>"><a href="<?php echo U('Role/configField',array('id'=>$this_role['id'],'type'=>1));?>">注册时填写资料配置</a></li>
            ...
            <div class="col-xs-2 text-right">
                <select name="role" class="form-control">
                    <?php if(is_array($role_list)): $i = 0; $__LIST__ = $role_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Role/configfield',array('id'=>$vo['id'],'type'=>1));?>" <?php if(($vo['id']) == $this_role['id']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="node-list in">
                <form action="/internship-tour/index.php?s=/admin/role/configfield/id/1/type/1.html" enctype="application/x-www-form-urlencoded" method="POST" class="form-horizontal auth-form">
                    <?php if(!empty($field_list)): if(is_array($field_list)): $i = 0; $__LIST__ = $field_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><dl class="checkmod">
                                <dt class="hd">
                                    <label class="checkbox"><input class="auth_fields fields_all" type="checkbox"><?php echo ($group["profile_name"]); ?></label>
                                </dt>
                                <dd class="bd">
                                    <?php if(isset($group['field_list'])): if(is_array($group['field_list'])): $i = 0; $__LIST__ = $group['field_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><div class="filed_check">
                                                <div>
                                                    <label class="checkbox" title="id：<?php echo ($field['id']); ?>  字段类型：<?php echo ($field['form_type']); ?>">
                                                        <input class="auth_fields fields_row" type="checkbox" name="fields[]" value="<?php echo ($field["id"]); ?>"/><?php echo ($field["field_name"]); ?>
                                                    </label>
                                                </div>
                        ...
                        <?php else: ?>
                        <div style="margin: 10px;color: rgb(255, 81, 81);">请先设置身份用户拥有的扩展资料字段</div><?php endif; ?>
...
<script type="text/javascript">
    ...
    <script type="text/javascript" charset="utf-8">
        ...
        //导航高亮
        highlight_subnav('<?php echo U('Role/configfield');?>');
        }(jQuery);
    </script>
